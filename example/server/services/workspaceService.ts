...
export const inviteToWorkspace = async (
  ...
  if (!invitee) throw new Error('Invitee not found');
   ...
   const plan = workspace.plan;  // Plan associated with the workspace
  ...
  sendInviteEmail(invitee.email, invitationLink, workspace.workspaceName);
...
export const acceptInvite = async (userId: string, workspaceId: string) => {
  const workspace = await Workspace.findById(workspaceId).exec();
  if (!workspace) throw new Error('Workspace not found');
...
// export const assignRole = async (adminId: string, userId: string, workspaceId: string, roleName: string) => {
...
//   if (!member) throw new Error('User not found in workspace');
